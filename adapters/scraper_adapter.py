...
class ScraperAdapter(ABC):
    ...
    # if not null, manager won't look in database for last but just get last date
    ...
    def set_last_scraped_date(self):
        '''
            ...
            if there was downtime for multiple days, we might be able to call history
            but if there was downtime for only a few hours:
    ...
    # filter data, remove BA, factories, etc, and combine production types in both functions
   ...
   Week or ANY time metric
    ...
    Question: Is it possible to push data into these 'day' arrays without reading them?
    ...
    def scrape_new_data(self) -> dict:
        '''
            Check if data is in database first?(probably needs a setter since this shouldn't be directly handling the DB)
            Scrape any new data (data that hasn't been passed before and based on the landing page)

            The goal of this function is for real-time data as opposed to general data collection
        '''
        pass
    ...
    def frequency(self) -> str:
        ...
        if downtime occurs, what is the solution? scrape_history? how to detect?
        manager(psudo name for main as of now) will check DB and determine missing info:
           - if missing:
